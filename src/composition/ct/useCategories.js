...
const createWhere = (categorySlug, rootOnly, locale) => {
  const where = [
    getValue(categorySlug) && getValue(locale)
      ? `slug(${getValue(locale)}="${getValue(categorySlug)}")`
      : false,
...
const useCategories = ({ locale, categorySlug, rootOnly, sort, skip }) => {
  ...
  useEffect(
    () =>
      setSkipQuery(
        getValue(skip) || (getValue(categorySlug) && !getValue(where))
      ),
